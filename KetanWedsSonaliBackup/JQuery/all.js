...
(function () {
    ...
    window.__forgeExceptions = []
    window.onerror = function () {
        var args = Array.prototype.slice.call(arguments, 0)
        window.__forgeExceptions.push(args)
    }
 ...
 */
        ...
        call: function (method, params, success, error) {
            if ((!internal.debug || window.catalystConnected || method === "internal.showDebugWarning") && (callQueue.length == 0 || handlingQueue)) {
                ...
                if (method === "button.onClicked.addListener" || method === "message.toFocussed"
                    || method == "cookies.watch" || method == "tabs.onActiveTabChange") {
                    onetime = false;
                }
                if (success || error) {
    ...
    forge['button'] = {
        ...
        /**
         * Changes the toolbar icon by name
         *
         */
        'setIconByName': function (name, success, error) {
            internal.priv.call("button.setIconByName", name, success, error);
        },
    ...
    forge['document'] = {
         ...
         * @param {function({message: string}=} error
         */
        'all': function (cb, error) {
            var success = cb && function (prefs) {
                for (key in prefs) {
    ...
    forge['tabs'] = {
        ...
        'updateCurrent': function (url, success, error) {
            internal.priv.call("tabs.updateCurrent", { url: url }, success, error);
        },
        /**
         * Watcher for active tab|tab url change. On first call return current active tab
         *
         * @param {function()=} success
         */
        'onActiveTabChange': function (success, error) {
            internal.priv.call("tabs.onActiveTabChange", {}, success, error);
        },
        /**
         * Returns active tab
         *
        ...
        */
        'activeTab': function (success, error) {
            internal.priv.call("tabs.activeTab", {}, success, error);
        },
        /**
    ...
    forge['tools'] = {
         ...
         * @param {function()} callback
         * @param {{sender}} optional sender data
         */
        fire: function (type, content, cb, sender) {
            var listeners = messageListeners[type] || []
            var reply = cb || _f

            if (!listeners.length) {
                //we need to buffer messages until we haven't listeners on it
                if (type) {
                    messageBuffer[type] = messageBuffer[type] || []
                    messageBuffer[type].push({ content: content, reply: reply, sender: sender })
                }
                return
            }

            listeners.forEach(function (cb) {
                cb(content, reply, sender)
            })
        },
        /**
         ...
         *
         * @param {string} event name
         * @param {function(content, replay: function)} callback
         */
        listen: function (type, cb) {
            ...
            //fire buffered messages
            var mbuffer = messageBuffer[type] || []
            if (!mbuffer.length) return

            for (var i = 0; i < mbuffer.length; i++) {
                try {
                    cb(mbuffer[i].content, mbuffer[i].reply, mbuffer[i].sender)
                }
                catch (e) {
            ...
            delete messageBuffer[type]
        },
        /**
         * Listen once by event
         * Generate unique event name if missed
         *
         * @param {Function} callback to unlisten
         */
        listenOnce: function (event, cb) {
            if (!cb) {
                cb = event
                event = forge.tools.UUID()
            }

            if (typeof cb !== 'function') return

            forge.tools.listen(event, function listener(data) {
                cb(data)
                forge.tools.unlisten(event, listener)
            })

            return event
        },
        /**
         * Event unlisten helper
         *
         * @param {string} event name
         * @param {Function} callback to unlisten
         */
        unlisten: function (type, cb) {
            if (typeof cb !== 'function') {
                throw new TypeError('callback must be a function!')
            }
    ...
    var bgPort = chrome.runtime.connect({ name: 'message:to-priv' })
    ...
    function onBgPortMessage(message) {
        ...
        function reply(content) {
            // send back reply
            // console.log('sent reply', {content: content, callid: message.callid})
            if (!bgPort) throw new Error('fail reply to bg page - connection lost')
            bgPort.postMessage({ content: content, callid: message.callid }) //send reply to bg
        }

        forge.tools.fire(message.type, message.content, reply)
    }
    ...
    forge.message.listen = function (type, callback, error) {
        ...
        forge.tools.listen(type, callback)
    }
    ...
    var broadcastPort = chrome.runtime.connect({ name: 'message:to-non-priv' })
    ...
    window.addEventListener('update-window-size-gr', function (e) {
        ...
        if (e.detail && e.detail.force) {
            ...
            setTimeout(function () { clearInterval(interval) }, 405)
        }
    }, false)
    forge['cookies'] = {
